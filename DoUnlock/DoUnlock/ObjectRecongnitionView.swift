...
struct ObjectRecongnitionView: View {
    ...
    // ObjectSimilarity는 DINO 모델과 저장 이미지 embedding을 들고 있는 비교 기능 객체입니다.
    @State private var objectSimilarity: ObjectSimilarity?

    // DINO와 저장 이미지 준비 상태, 프레임 비교 중복 실행 방지 상태, 최근 결과를 화면에 보여주기 위한 상태들입니다.
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            // 하단에는 현재 유사도와 가장 유사한 저장 도어락 이름을 보여줍니다.
    ...
    private func prepareSimilarity() {
        ...
        do {
            // 화면이 뜰 때 DINO 모델을 로드하고 SwiftData 저장 이미지 embedding을 미리 만들어둡니다.
    ...
    private func handleComparisonSuccess(
        ...
        // 비교 성공 시 점수와 가장 비슷한 저장 도어락 이름을 저장하고, 프레임 색상을 갱신합니다.
        isComparingFrame = false
        ...
        if let bestMatchName = similarityResult.bestMatchName {
            print(
                "Best registered match: \(bestMatchName), score: \(similarityResult.score)"
            )
        }
    ...
    private func handleComparisonFailure(_ message: String) {
        // 모델, 저장 이미지, crop 변환 중 문제가 생기면 빨간 프레임과 안내 문구로 상태를 보여줍니다.
        isComparingFrame = false
